...
export default function CategoryPage(props: {
  ...
  const filteredProducts = products.filter((p) => {
    ...
    if (slug === "gold-jewellery") {
      ...
      const pSub = p.subCategory.toLowerCase().replace(/[^a-zA-Z0-9]/g, "");
      const cleanSlug = slug.toLowerCase().replace(/[^a-zA-Z0-9]/g, "");
      // Match singular/plural variances
      const matchSub = pSub === cleanSlug || 
                       pSub.replace("s", "") === cleanSlug.replace("s", "") ||
                       pSub.includes(cleanSlug) ||
                       cleanSlug.includes(pSub);
